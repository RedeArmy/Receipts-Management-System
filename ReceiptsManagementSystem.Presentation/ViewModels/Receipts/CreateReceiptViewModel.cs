...
public sealed partial class CreateReceiptViewModel : BaseViewModel
{
    ...
    [ObservableProperty] private bool _showCheckFields    = false;
    [ObservableProperty] private bool _showTransferFields = false;
    ...
    private async Task SaveReceiptAsync()
    {
        await ExecuteBusyAsync(async () =>
        {
            ...
            {
                CustomerId            = Guid.NewGuid(), // temporal hasta tener Customer
            ...
            var id      = await _mediator.Send(command, CancellationToken.None);

            SuccessMessage = $"Recibo creado exitosamente.";
            ClearForm();
        });
    }
